...
st.title("BRT VisaScore: Sua Chance de Visto B1/B2")
...
if st.button("ANALISAR PERFIL"):
    ...
    if emprego == "Selecione aqui..." or tempo == "Selecione aqui..." or renda == 0 or None in campos_radio:
        ...
        # Pontuação Refinada
        if emprego == "CLT": score += 30; positivos.append("Vínculo CLT")
        elif emprego == "Empresário": score += 35; positivos.append("Empresário")
        elif emprego == "Autônomo": score += 15; positivos.append("Autônomo")
        else: score -= 20; negativos.append("Ausência de vínculo profissional")

        if tempo == "Mais de 3 anos": score += 20
        elif tempo == "1 a 3 anos": score += 10
        
        if renda >= 15000: score += 30
        elif renda >= 7000: score += 20
        
        if filhos == "Sim": score += 15
        if imovel == "Sim": score += 25; positivos.append("Possui imóvel")
        if viagens == "Sim": score += 20
        if visto_usa == "Sim": score += 30; positivos.append("Histórico de visto americano")
        if negado == "Sim": score -= 50; negativos.append("Histórico de visto negado")

        st.divider()
        # Escala ajustada para perfis sólidos
        if score >= 100: st.balloons(); st.success(f"Chance ALTA de Aprovação (Score: {score})")
        elif score >= 70: st.success(f"Boa chance de aprovação (Score: {score})")
        elif score >= 40: st.warning(f"Chance moderada (Score: {score})")
