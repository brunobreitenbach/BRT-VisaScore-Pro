...
with col1:
    # Nome com placeholder
    nome = st.text_input("Nome", placeholder="Preencha aqui")
    # Idade iniciada vazia (usando None ou um valor neutro)
    idade = st.number_input("Idade", min_value=18, max_value=90, value=None)
    # index=None deixa o selectbox vazio
    estado_civil = st.selectbox("Estado civil", ["Solteiro(a)", "Casado(a)", "Divorciado(a)", "Viúvo(a)"], index=None)
    emprego = st.selectbox("Tipo de emprego", ["CLT", "Autônomo", "Empresário", "Desempregado"], index=None)
...
with col2:
    ...
    proposito = st.selectbox("Propósito da viagem", ["Turismo", "Estudos", "Negócios"], index=None)
